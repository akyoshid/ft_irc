...
void Server::acceptNewConnection() {
    ...
    while (1) {
        ...
        int clientFd
            = accept(
                serverSocket_, (struct sockaddr*)&clientAddr, &clientAddrLen);
        if (clientFd < 0) {
...
void Server::recvFromClient(Client* client) {
    ...
    while (true) {
        ...
        if (bytesRead > 0) {
            ...
            client->readBuffer_.erase(
                std::remove(client->readBuffer_.begin(),
            ...
            if (client->readBuffer_.size() > MAX_BUFFER_SIZE) {
                log(LOG_LEVEL_ERROR, LOG_CATEGORY_CONNECTION,
                    "Read buffer is too large, try to disconnect: "
                    + client->ip_);
...
void Server::sendToClient(Client* client) {
    ...
    while (!client->writeBuffer_.empty()) {
        ssize_t bytesSent
            = send(client->socketFd_, client->writeBuffer_.c_str(),
