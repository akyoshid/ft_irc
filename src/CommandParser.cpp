#include "CommandParser.hpp"

#include <string>

#include "User.hpp"
#include "utils.hpp"

CommandParser::CommandParser() {}

CommandParser::~CommandParser() {}

void CommandParser::processMessage(User* user, const std::string& message) {
  (void)this;  // Suppress unused warning
  // Stub implementation: Just log the message
  log(LOG_LEVEL_INFO, LOG_CATEGORY_COMMAND, user->getIp() + ": " + message);

  // TODO(unknown): Future implementation will:
  // 1. Parse the IRC command (PASS, NICK, USER, JOIN, etc.)
  // 2. Validate command syntax
  // 3. Check user authentication/registration status
  // 4. Execute the appropriate command handler
  // 5. Send response to user

  // Example future structure:
  // Command cmd = parse(message);
  // if (!validate(cmd, user)) return sendError(user, ...);
  // execute(cmd, user);
}
